...
//		font.setItalic(true);
		...
		p.drawText(this->rect().x(),this->rect().y(),this->rect().width(),this->rect().height(),Qt::AlignTop|Qt::AlignCenter,m_tipstr);
	}
}

void Slider::SetTipStr(QString tipstr)
{
	m_tipstr = tipstr;
	if(m_showtip) update();
}
...
void Slider::mousePressEvent(QMouseEvent *e)
{
	if (e->button() == Qt::LeftButton) {
        ...
        if (!sliderRect.contains(e->pos())) {
            e->accept();
			//不知道为什么这里的计算方法和move函数里的计算方法不一样，没必要，而且会有偏差。
